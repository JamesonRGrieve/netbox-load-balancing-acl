...
class LBMemberHAForm(NetBoxModelForm):
    ...
    fieldsets = (
        ...
        FieldSet("ssl_client_cert", "ssl_verify", "member_port", name="Backend mTLS (authenticated origin)"),
    )

    class Meta:
        model = LBMemberHA
        fields = [
            "assignment", "backup", "ssl_client_cert", "ssl_verify", "member_port", "description", "tags",
        ]
