...
(the frontend the ACL lives on) to a ``netbox_load_balancing.VirtualIPPool`` (the backend
pool matching traffic routes to), via a match type + pattern — the HAProxy ACL that
``netbox_load_balancing`` itself does not model, so it no longer falls back to ``config_context``."""
...
class LBRoutingRule(NetBoxModel):
    ...
    target_pool = models.ForeignKey(
        ...
        related_name="routing_rules",
        help_text="Backend pool matching traffic routes to.",
    )
